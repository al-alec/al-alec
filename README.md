<img src="https://images.unsplash.com/photo-1566837945700-30057527ade0?ixlib=rb-1.2.1&auto=format&fit=crop&w=1170&q=80" width="100%" style="border-radius:10px;"/>

# Hello! Welcome to my Profile <img align="right" src="https://komarev.com/ghpvc/?username=al-alec&style=flat-square&color=blueviolet">

## About Me

I'm **Alexis Yehadji**, a **Full-Stack Developer** and **AI researcher** passionate about building reliable, scalable, and data-driven systems.  
I currently work at **Institut Pasteur (Paris)**, focusing on deep learning and biomedical image analysis.

**Mission:** Build intelligent systems that connect software engineering and applied AI.  
**Philosophy:** Think precisely. Code cleanly. Deliver impact.  
**Location:** Lomé (Togo) → Paris (France)

---

## Areas of Expertise

- Web Engineering: Architecture, APIs, and backend systems for scalable apps.  
- Data Science and Machine Learning: End-to-end pipelines and neural network design.  
- MLOps and DevOps: Automating training, testing, and deployment with Docker and Kubernetes.  
- Open Source Development: Writing reusable Python and JavaScript libraries with documentation.

---

## Selected Open Source Projects

### Wav2Vec-BERT Audio Sentiment Analyzer  
[GitHub Repository](https://github.com/al-alec/audio-sentiment-analyzer)

End-to-end system for audio emotion recognition using Wav2Vec 2.0 for transcription and BERT for sentiment analysis.  
Includes a Gradio interface, API, and real-time inference demo.

**Stack:** PyTorch, Transformers, Gradio, FastAPI, NumPy

---

### ioumatch  
[PyPI Package](https://pypi.org/project/ioumatch/)

Python library published on PyPI for computing IoU-based matching metrics in segmentation tasks.  
Lightweight and NumPy-based, designed for binary and multi-label masks.

**Stack:** NumPy, OpenCV, Scikit-Image, Python Packaging

---

## Technology Stack

### Languages and Frameworks
<p align="center">
  <img src="https://skillicons.dev/icons?i=python,php,laravel,js,ts,vue,nuxt,fastapi,pytorch" height="55"/>
</p>

### Databases and Caching
<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,postgresql,mongodb,redis" height="55"/>
</p>

### DevOps and Cloud
<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,kubernetes,aws,nginx,git,githubactions" height="55"/>
</p>

### Frontend and UI/UX
<p align="center">
  <img src="https://skillicons.dev/icons?i=tailwind,vuetify" height="55"/>
</p>

### Data Science and Machine Learning Tools
<p align="center">
  <img src="https://skillicons.dev/icons?i=tensorflow,pytorch,scikitlearn,jupyter,opencv" height="55"/>
</p>

---

## GitHub Analytics

<p align="center">
  <img height="160em" src="https://github-readme-stats.vercel.app/api?username=al-alec&show_icons=true&theme=dracula&count_private=true" />
  <img height="160em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=al-alec&layout=compact&theme=dracula&count_private=true" />
</p>

---

## Contact

<p align="center">
  <a href="https://linkedin.com/in/alexis-honore-yehadji">
    <img src="https://img.shields.io/badge/-Alexis%20Yehadji-0077B5?style=flat&logo=Linkedin&logoColor=white"/>
  </a>
  <a href="mailto:yehadjialexis@gmail.com">
    <img src="https://img.shields.io/badge/-yehadjialexis@gmail.com-D14836?style=flat&logo=Gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/al-alec">
    <img src="https://img.shields.io/badge/-GitHub-181717?style=flat&logo=github&logoColor=white"/>
  </a>
  <a href="https://pypi.org/user/al-alec/">
    <img src="https://img.shields.io/badge/-PyPI-3775A9?style=flat&logo=pypi&logoColor=white"/>
  </a>
</p>

---

*Open source believer, research-driven developer, and lifelong learner.*
